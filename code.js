    ...
    function generate() {
      ...
      let getIterateLines = (arr, className, key, legalKey, jsonKey) => {
        ...
        if (inner === undefined || inner === null) {
          let warningInfo = `WARNING : the property named &nbsp <b>'${key}'</b> &nbsp is an EMPTY array ! parse process is failed !`
          showInfo(warningInfo);
          return { fromJsonLinesJoined: `    // ${warningInfo}\n`, toJsonLinesJoined: `    // ${warningInfo}\n` };
        }
        let total = count;
        let fromJsonLines = [];
        let toJsonLines = [];
      ...
      let dartCode = `///\n/// Code generated by jsonToDartModel http://www.eyrefree.org/jsonToDartModel///\n///\n${objToDart(jsonObj, rootClass, "")}`;
